...
@section('title', 'Edit Kota - Admin Panel')
...
<div class="min-h-screen py-8 md:py-12">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl sm:text-4xl font-bold mb-2 text-white">Edit Kota Pembanding</h1>
            <p class="text-gray-400">Edit data kota pembanding</p>
        </div>

        <!-- Form Card -->
        <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 md:p-8 shadow-xl">
            <form action="{{ route('admin.kota.update', $kota->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input 
                            ...
                            id="umr" 
                            name="umr" 
                            value="{{ old('umr', $kota->umr) ? number_format(old('umr', $kota->umr), 0, ',', '.') : '' }}"
                            required
                            inputmode="numeric"
                            pattern="[0-9.,]*"
                            class="w-full pl-12 pr-4 py-3 bg-slate-900/50 border border-slate-600 text-gray-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all duration-200 @error('umr') border-red-500 @enderror"
                        >
                    </div>
                    @error('umr')
                        <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input 
                            ...
                            id="waktu_tempuh" 
                            name="waktu_tempuh" 
                            value="{{ old('waktu_tempuh', $kota->waktu_tempuh) ? number_format(old('waktu_tempuh', $kota->waktu_tempuh), 0, ',', '.') : '' }}"
                            required
                            inputmode="numeric"
                            pattern="[0-9.,]*"
                            class="w-full px-4 py-3 pr-20 bg-slate-900/50 border border-slate-600 text-gray-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all duration-200 @error('waktu_tempuh') border-red-500 @enderror"
                        >
                ...
                <div>
                    ...
                    <input 
                        ...
                        id="jumlah_armada" 
                        name="jumlah_armada" 
                        value="{{ old('jumlah_armada', $kota->jumlah_armada) ? number_format(old('jumlah_armada', $kota->jumlah_armada), 0, ',', '.') : '' }}"
                        required
                        inputmode="numeric"
                        pattern="[0-9.,]*"
                        class="w-full px-4 py-3 bg-slate-900/50 border border-slate-600 text-gray-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all duration-200 @error('jumlah_armada') border-red-500 @enderror"
                    >
                    @error('jumlah_armada')
                        <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input 
                        ...
                        value="{{ old('jumlah_kendaraan_pribadi', $kota->jumlah_kendaraan_pribadi) ? number_format(old('jumlah_kendaraan_pribadi', $kota->jumlah_kendaraan_pribadi), 0, ',', '.') : '' }}"
                        required
                        inputmode="numeric"
                        pattern="[0-9.,]*"
                        class="w-full px-4 py-3 bg-slate-900/50 border border-slate-600 text-gray-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all duration-200 @error('jumlah_kendaraan_pribadi') border-red-500 @enderror"
                    >
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input 
                            ...
                            value="{{ old('tarif_minimum_aktual', $kota->tarif_minimum_aktual) ? number_format(old('tarif_minimum_aktual', $kota->tarif_minimum_aktual), 0, ',', '.') : '' }}"
                            required
                            inputmode="numeric"
                            pattern="[0-9.,]*"
                            class="w-full pl-12 pr-4 py-3 bg-slate-900/50 border border-slate-600 text-gray-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all duration-200 @error('tarif_minimum_aktual') border-red-500 @enderror"
                        >
                    </div>
